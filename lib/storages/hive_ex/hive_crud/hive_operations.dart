...
class _HiveTodoState extends State<HiveTodo> {
          ...
          itemBuilder: (context, index) {
            return Card(
              ...
              child: Column(
                ...
                children: [
                  Text(task[index]['taskname'],style: GoogleFonts.alatsi(fontWeight: FontWeight.bold),),
                  ...
                  Expanded(child: Row(children: [
                    IconButton(onPressed: ()=>showAlertbox(task[index]['id']),//open alertbox which create before
  ...
  Future <void> createTeask(Map<String, dynamic> mytask) async{
    ...
    // mytask['createdTime']=now.toString();
    await my_box.add(mytask);
    readTask_refreshUi(); //refresh ui after each creation,deleton ,update
  }

   readTask_refreshUi(){
    final task_from_hive=my_box.keys.map((key){  //fetch all the keys fro hive box  //to read from hive
    ...
    setState(() {
      ...
      task=task_from_hive.reversed.toList();  //sort in normal they sort as alphebetic order but we need ascending order bcz this is todoo notepad so we use this "reversed"
    });
